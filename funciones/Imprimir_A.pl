...
# Archivos de input
$in_salas = "$GRUPO/$MAEDIR/"."salas.mae";	#id (n° par);nombre;capacidad;direccion;telefono;email
$in_obras = "$GRUPO/$MAEDIR/"."obras.mae";	#id (n° impar);nombre;email produccion general;email produccion ejecutiva
$in_reservas_confirmadas = "$GRUPO/$PROCDIR/"."reservas.ok";	#id obra;nombre;fecha;hora;id sala;nombre sala;cant butacas confir;id combo[;ref int];cant butacas solicitadas;email;usuario;fecha grabacion
$in_reservas_no_confirmadas = "$GRUPO/$PROCDIR/"."reservas.nok";   #ref int;fecha;hora;numero fila;numero butaca;cant butacas solicitadas;seccion;motivo;id sala;id obra;email;usuario;fecha grabacion
$in_disponibilidad = "$GRUPO/$PROCDIR/"."combos.dis"; #id combo;id obra;fecha;hora;id sala;butacas habilitadas;butacas disp;requisitos especiales
...
sub generar_listado_eventos_candidatos {
	...
	# $in_reservas_confirmadas = "$GRUPO/$PROCDIR/"."reservas.ok";	
...
sub escribir_listado_disponibilidad {
	local $out_disponibilidad = "$GRUPO/$REPODIR"."/"."$_[0]".".dis";
	if ($write == 1)
	{
...
sub escribir_listado_invitados {
	local $out_invitados_confirmados = "$GRUPO/$REPODIR"."/"."$evento".".inv";
	local $archivo;
...
sub escribir_listado_tickets {
	local $out_tickets = "$GRUPO/$REPODIR"."/"."$_[0]".".tck";
...
sub escribir_ranking {
	...
	$nnn=`ls "$GRUPO/$REPODIR" | grep "^ranking" | sed s/ranking\.//g | sort -r | head -n 1`;
	...
	$out_ranking = "$GRUPO/$REPODIR"."/ranking."."$nnn";
	if ($write == 1)
	{
...
sub fInvitados {
	...
	foreach $ref_int (keys(%referencias_internas))
	{
		if ($referencias_internas{$ref_int} eq $evento)
		{
			...
			local $nom_archivo_invitados = "$GRUPO/$REPODIR/"."$ref_int".".inv";
