...
then
	...
	then
		...
		then
				...
				if [ ! -f "$GRUPO/${DIRECTORIO_MAESTROS}/${ARCHIVOS_MAESTROS[${aux}]}" ]
						...
						then
								...
								echo "Fueron seteados permisos. No había permisos de lectura sobre el archivo maestro: ${DIRECTORIO_MAESTROS}/${ARCHIVOS_MAESTROS[${aux}]}."
						fi
				done
		fi
		...
		then
			...
			then
				echo " "
				echo "ERROR: El proceso Recibir_A ya se esta ejecutando con el PID $PID_RECIBE."
